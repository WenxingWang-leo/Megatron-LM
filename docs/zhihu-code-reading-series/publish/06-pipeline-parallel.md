...
> **读本节前先记住一句话：**  
> 下面先出现的「`F3 B0 F4 B1 …`」是**单个 stage 自己的任务清单（本地指令序）**，  
> **不是**「这些操作在全局同一时刻一起发生」。  
> 全局何时真正执行，看后面的**墙钟时间线**（中间会插入空等 `·`）。

#### A. 本地指令序 = 这个 rank 的 for 循环要干什么

源码三阶段（`forward_backward_pipelining_without_interleaving`）：

```text
warm = min(m, PP - rank - 1)
rem  = m - warm

① warmup:   连续做 warm 次 Forward（还不动 Backward）
② steady:   循环 rem 次：先 Forward 一个新 microbatch，再 Backward 队列里最老的那个
③ cooldown: 再做 warm 次 Backward（清空剩下的）
```

对 `PP=4, m=8`，四个 stage 各自的任务清单是：

```text
         |← warmup →|←-------- 1F1B 稳态（本地交替）--------→|← cooldown →|
...
怎么读 s0 的 `F3 B0`（最容易困惑的一点）：

```text
含义：s0 的代码顺序是「先安排做 F3，再安排做 B0」
     ——因为 warmup 已经做过 F0/F1/F2，稳态第一次 Backward 弹出的是最老的 F0 → 即 B0。

不含义：F3 和 B0 在同一时刻完成，或 B0 在 s3 做完 F0 之前就开始。
```

同理：**s3 的清单是 `F0 B0 F1 B1 …`**（warmup=0，进来就是 1F1B），  
**不是** GPipe 那种 `F0 F1 … F7` 再 `B0 … B7`。

#### B. 墙钟时间线 = 加上 P2P 等待之后，大家真正何时干活

约束只有两条（每格假定 1 次 F 或 1 次 B）：

```text
· 某 stage 做 Fi 之前：必须先收到上一个 stage 的 Fi 激活（s0 除外）
· 某 stage 做 Bi 之前：必须先收到下一个 stage 的 Bi 梯度（s3 在自己 Fi 之后立刻可以 Bi）
```

因此同一条「本地相邻」的 `F3 B0`，在墙上钟会被拆开：
...
| 本地清单里 | 墙钟上实际 |
|------------|------------|
| `… F2 \| F3 B0 …` 写在一起 | `F3` 在 t=3，然后 **空等 t=4,5,6**，`B0` 才到 t=7 |
| 看起来像 F3 后立刻 B0 | 立刻只是「下一道工序」；工序要等梯度从 s3 传回来 |
...
t=5  s2 做 B0
t=6  s1 做 B0
t=7  s0 做 B0   ← 第一条反向浪涌到最前 stage
```

所以：**看清单时不要竖着对齐各 stage 的 `B0`；要对齐，请只看墙钟表的同一列 `t`。**

#### C. 两张图表怎么配合着用

```text
问「这个 stage 接下来该做 F 还是 B、编号是几？」 → 看 A 本地清单
问「会不会和别的 stage 冲突？气泡在哪？」     → 看 B 墙钟表
问「清单里 F3 紧挨 B0，是不是同时发生？」   → 不是；墙钟里中间有 ·
```

气泡主要就是墙钟表里的 `·`：前排等第一条反向（s0 的 t=4..6），以及收尾时前排等后排把剩余 `B` 传完。
