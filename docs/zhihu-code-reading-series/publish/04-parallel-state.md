...
所以若你期待「每组 4 个 rank」，那对应的是 **`DP=4`**，不是本例的 `DP=2`。  
注意公式是 **`DP = world / (TP×PP×CP)`**，不是 `world/TP` 或 `world/1`：

| 配置 | DP（每组大小） | DP 组个数 `world/DP` |
|------|----------------|----------------------|
| world=8, TP=2, PP=2, CP=1（本例） | `8/(2×2×1)=2` | 4 |
| world=8, TP=2, PP=1, CP=1 | `8/(2×1×1)=4` | 2 |
| world=8, TP=1, PP=1, CP=1 | `8/(1×1×1)=8` | 1 |

（上一版误写成「world=8 且 TP=PP=1 → DP=4」，那是错的；该配置下 DP=8。）
